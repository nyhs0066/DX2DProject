...
bool MyWriter::init()
{
	szTargetString = L"테스트 텍스트";
	
	FLOAT RBX = (float)MyWindow::cWidth;
	FLOAT RBY = (float)MyWindow::cHeight;
	FLOAT LTX = RBX / 2.0f - 30;
	FLOAT LTY = RBY / 2.0f - 30;

	position = { LTX, LTY, RBX, RBY };
...
bool MyWriter::render()
{
	...
	pD2DRenderTarget->DrawText(szTargetString.c_str(), szTargetString.size(),
		pTextFormat, position, pSColorBrush);

	pTextLayout->SetUnderline(TRUE, { 2, 1 });
	pD2DRenderTarget->DrawTextLayout({ 100.0f, 100.0f }, pTextLayout, pSColorBrush);
...
bool MyWriter::draw(PCWSTR szString, FLOAT stX, FLOAT stY)
{
	D2D_RECT_F pos = { stX, stY, (float)MyWindow::cWidth, (float)MyWindow::cHeight };

	pD2DRenderTarget->BeginDraw();

	pD2DRenderTarget->DrawText(szString, wcslen(szString),
