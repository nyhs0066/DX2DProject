...
FMOD_RESULT MySound::setVolume(UINT iVolume)
{
	FMOD_RESULT fr = FMOD_ERR_CHANNEL_ALLOC;
	...
	if (m_pChannel)
	{
		...
		volume = max(0.0f, volume / 100.0f);
		volume = min(1.0f, volume / 100.0f);
		fr = m_pChannel->setVolume(volume);
	}
...
MySound* MySoundManager::load(wstring wszFileName)
{
	MySound* ret = nullptr;
	...
	if (it == m_soundMap.end())
	{
		...
		if (ret->create(m_pSystem, api_W2M(wszFileName.c_str())))
		{
			ret->release();
			delete ret;
			ret = nullptr;
