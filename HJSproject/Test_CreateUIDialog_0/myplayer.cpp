...
MyVector2F MyMovementTestDummy::staticCollisionHandler(const MyObject2D* pCObj, const MyRect& cRect)
{
	...
	if (MyCollision::rectToRect(temp, CObjRect))
	{
		...
		{
			m_SPRect.pos.x = CObjRect.pos.x - m_SPRect.fWH.v[0] * 0.5f - 0.05f;
		}
		else if (m_CSRect.pos.x - m_prevCSRect.pos.x < 0.0f)
		{
			m_SPRect.pos.x = CObjRect.pos.x + CObjRect.fWH.v[0] + m_SPRect.fWH.v[0] * 0.5f + 0.05f;
		}
	}
	...
	if (MyCollision::rectToRect(temp, CObjRect))
	{
		...
		{
			m_SPRect.pos.y = CObjRect.pos.y - m_SPRect.fWH.v[1] * 0.5f - 0.05f;
		...
		{
			m_SPRect.pos.y = CObjRect.pos.y + CObjRect.fWH.v[1] + m_SPRect.fWH.v[0] * 0.5f + 0.05f;
			m_vVelocity.y = 0.0f;
		}
	}
