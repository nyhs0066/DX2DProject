...
bool MyInGameScene::frame()
{
	...
	if (MAININPUT.getKeyState(VK_LBUTTON))
	{
		...
		if (dummy.m_fAttackTimer > 1.0f / dummy.m_fAttackSpeed)
		{
			...
			dummy.createEffect(ePresetList[1], effectList, nullptr);
			dummy.playShotSound();
		}
	}
	...
	for (eit = enemyList.begin(); eit != enemyList.end(); eit++)
	{
		...
		{
			(*eit)->createEffect(ePresetList[1], effectList, &dummy);
			(*eit)->m_state = ES_IDLE;
		}
...
void MyInGameScene::randomEnemySpawn()
{
	...
	if ((enemySpawnTimer >= 1.0f) && (enemyList.size() <= MAX_ENEMY_NUMBER))
	{
		MYRND_IRANGE(1, 3);
