...
bool MyInGameScene::init()
{
	...
	//BOSS set
	pRect = { 100.0f, 800.0f, 200.0f, 400.0f };
	Left.create(rPaths[0] + L"mydefaultshader.txt", rPaths[2] + L"Square.png",
		pRect, BoundaryRect);
	Left.pTarget = &dummy;
	Left.init();

	pRect = { 2300.0f, 800.0f, 200.0f, 400.0f };
...
bool MyInGameScene::frame()
{
	...
	if (Left.m_bLifeFlag) 
	{ 
		if (Left.m_bDAFlag) { Left.createEffect(ePresetList[1], enemyEffectList, &dummy); } 
	}
	if (Right.m_bLifeFlag)
	{
		if (Right.m_bDAFlag) { Right.createEffect(ePresetList[1], enemyEffectList, &dummy); }
	}
