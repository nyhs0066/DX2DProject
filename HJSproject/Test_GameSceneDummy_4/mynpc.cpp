...
MyFirstBoss::MyFirstBoss()
{
	for (int i = 0; i < 6; i++)
	{
		...
		m_airHelper[i].m_fTimeLimit = 3.0f;
	...
	m_defHelper[0].m_fProjSize = 75.0f;
	...
	m_defHelper[1].m_fProjSize = 100.0f;
	...
	m_defHelper[2].m_fProjSize = 125.0f;
...
bool MyFirstBoss::init()
{
	...
	m_shield.m_iMaxHitPoint = 1000.0f;
	m_shield.m_iCurHitPoint = 1000.0f;
...
bool MyFirstBoss::frame()
{
	...
	if (m_fDefenseTimer > m_fDefenseTime)
	{
		...
		m_fDefenseTimer = 0.0f;
		m_shield.m_bLifeFlag = TRUE;
		m_shield.m_iCurHitPoint = m_iMaxHitPoint;
	}
	...
	if (m_bState == BS_DIRECT_ATTACK) { m_bDAFlag = TRUE; }
