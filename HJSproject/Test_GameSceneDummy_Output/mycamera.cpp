...
MyCamera::MyCamera()
{
    ZeroMemory(&m_Camera, sizeof(MyExRect));
...
bool MyCamera::frame()
{
    ...
    {
        //이미 좌표가 중앙에 와있다.
        MyExRect fRect = m_pFollowing->m_SPRect;
...
void MyCamera::setBoundaryRect(const MyExRect& bRect)
{
    m_bRect = bRect;
}
...
void MyCamera::setCamRect(const MyExRect& rt)
{
    m_Camera = rt;
}
...
MyExRect MyCamera::getCamRect() const
{
    return m_Camera;
}
