...
bool MyEffect::update()
{
    ...
    if (m_fStepT <= m_fTimer)
    {
        ...
        if (m_pEPreset->m_pSprite->m_rtSpriteArray.size())
        {
            if (m_iCuridx >= m_pEPreset->m_pSprite->m_rtSpriteArray.size()) { m_iCuridx = 0; }
            m_TRect = m_pEPreset->m_pSprite->m_rtSpriteArray[m_iCuridx];
        }
        else
        {
            if (m_iCuridx >= m_pEPreset->m_pSprite->m_texSpriteArray.size()) { m_iCuridx = 0; }
            m_TRect = m_pEPreset->m_pSprite->m_texSpriteArray[m_iCuridx]->m_tRect;
        }
    }
...
void MyEffect::setTimeProperty(FLOAT fLifeSpanT, FLOAT fPlayT)
{
    ...
    if (m_pEPreset->m_pSprite->m_rtSpriteArray.size())
    {
        m_fStepT = fPlayT / m_pEPreset->m_pSprite->m_rtSpriteArray.size();
    }
    else
    {
        m_fStepT = fPlayT / m_pEPreset->m_pSprite->m_texSpriteArray.size();
    }
}
