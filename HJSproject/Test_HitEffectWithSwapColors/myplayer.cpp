...
bool MyMovementTestDummy::frame()
{
	...
	if (MAININPUT.getKeyState(VK_SPACE))
	{
		...
		else if(m_iEnableJumpNum && !m_upFlag)
		{
			m_fElapseJumpTime = 1.0f;
			m_upFlag = TRUE;
			m_iEnableJumpNum--;
...
bool MyMovementTestDummy::boundaryCheck()
{
	...
	{
		ret = false;
		m_SPRect.pos.y = m_boundary.fWH.v[1] - m_SPRect.fWH.v[1] * 0.5f - 0.001f;
		m_vVelocity.y = 0.0f;
		if (m_aState & MYPS_JUMP) { m_aState -= MYPS_JUMP; m_iEnableJumpNum = MAX_JUMP_COUNT; }
	}
...
MyVector2F MyMovementTestDummy::staticCollisionHandler(const MyObject2D* pCObj, const MyRect& cRect)
{
	...
	if (MyCollision::rectToRect(temp, CObjRect))
	{
		...
		{
			m_SPRect.pos.y = CObjRect.pos.y - m_SPRect.fWH.v[1] * 0.5f - 0.001f;
			m_vVelocity.y = 0.0f;
			if (m_aState & MYPS_JUMP) { m_aState -= MYPS_JUMP; m_iEnableJumpNum = MAX_JUMP_COUNT; }
