...
bool MyPlayer2D::frame()
{
	...
	if (MAININPUT.getKeyState('D') != KEY_FREE)
	{
		...
		if (m_SPRect.pos.x > m_BRect.fWH.v[0] - m_SPRect.fWH.v[0])
		{ 
			m_SPRect.pos.x = m_BRect.fWH.v[0] - m_SPRect.fWH.v[0];
		}
		setPosRect(m_SPRect);
	...
	if(m_aState == MYPS_JUMP)
	{
		...
		if (m_SPRect.pos.y > (FLOAT)m_BRect.fWH.v[1] * 3 / 4 - 50.0f)
		{
			m_SPRect.pos.y = (FLOAT)m_BRect.fWH.v[1] * 3 / 4 - 50.0f;
			m_actionFlag = FALSE;
...
bool MyMovementTestDummy::postFrame(const MyRect& camRt, FLOAT radian)
{
...
MyVector2F MyMovementTestDummy::staticCollisionHandler(const MyObject2D* pCObj, const MyRect& cRect)
{
	...
	MyRect CObjRect = pCObj->getCSRect();
