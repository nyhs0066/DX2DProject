...
QuadTree::QuadTree(MyExRect rect, int maxDepth)
{
	this->maxDepth = maxDepth;
	create(rect);
}
...
void QuadTree::create(FLOAT x, FLOAT y, FLOAT width, FLOAT height)
{
	if(!pRoot)
	{
		MyExRect nPos(x, y, width, height);
...
void QuadTree::create(MyExRect rt)
{
	if (!pRoot)
	{
		pRoot = createNode(rt);
...
QuadTreeNode* QuadTree::createNode(MyExRect nPos, int depth)
{
...
QuadTreeNode* QuadTree::findPutNodeSub(QuadTreeNode* pTarget, MyObject2D* pBObj)
{
	...
	if (MyCollision::rectToRect(pTarget->nPos, pObj->getCSRect()) == MYCT_IN)
	{
		pFind = pTarget;
...
void QuadTree::getStaticCollisionList(const MyObject2D* pBObj, vector<MyObject2D*>& objList)
{
	const MyObject2D* pDObj = pBObj;

	if (pDObj) { getStaticCollisionListSub(pDObj, pRoot, objList); }
}

void QuadTree::getStaticCollisionListSub(const MyObject2D* pDObj, const QuadTreeNode* pTarget,
	vector<MyObject2D*>& objList)
{
	for (int i = 0; i < pTarget->staticObjList.size(); i++)
	{
		if (MyCollision::rectToRect(pDObj->getCSRect(), pTarget->staticObjList[i]->getCSRect()))
		{
	...
	{
		if (MyCollision::rectToRect(pTarget->pChild[i]->nPos, pDObj->getCSRect()))
		{
			getStaticCollisionListSub(pDObj, pTarget->pChild[i], objList);
		}
	}
}

void QuadTree::getDynamicCollisionList(const MyObject2D* pBObj, vector<MyObject2D*>& objList)
{
	const MyObject2D* pDObj = pBObj;

	if (pDObj) { getDynamicCollisionListSub(pDObj, pRoot, objList); }
}

void QuadTree::getDynamicCollisionListSub(const MyObject2D* pDObj, const QuadTreeNode* pTarget,
	vector<MyObject2D*>& objList)
{
	for (int i = 0; i < pTarget->dynamicObjList.size(); i++)
	{
		if (MyCollision::rectToRect(pDObj->getCSRect(), pTarget->dynamicObjList[i]->getCSRect()))
		{
	...
	for (int i = 0; i < pTarget->pChild.size(); i++)
	{
		if (MyCollision::rectToRect(pTarget->pChild[i]->nPos, pDObj->getCSRect()))
		{
			getDynamicCollisionListSub(pDObj, pTarget->pChild[i], objList);
		}
	}
}
