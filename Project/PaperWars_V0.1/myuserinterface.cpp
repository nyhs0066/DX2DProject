...
{
	S_button_hover = LOAD_SOUND(L"../../data/sound/button_hover.mp3");
	S_button_Select = LOAD_SOUND(L"../../data/sound/button_select.wav");
}
	...
	if (m_bAvailableFlag)
	{
		if (MyCollision::pointToRect(fPt, m_SPRect))
		{
			...
			if (input)
			{
				...
				{ 
					m_bState = UB_SELECT; 
					S_button_Select->playEffect();
				}
			}
			else 
			{ 
				if (m_bState != UB_HOVER) { S_button_hover->playEffect(); }
				m_bState = UB_HOVER;
			}
		}
